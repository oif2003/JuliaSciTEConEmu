...
if FileExist(configFile) {
	juliaPath := IniRead(configFile, "path", "JuliaPath")
	if !FileExist(juliaPath)
		updatePath()
} else
	updatePath()
...
while true {
	if !WinExist("ahk_pid " scite.pid) {
		WinClose("ahk_pid " julia.ParentPID)
		ExitApp()
	}
...
updatePath() {
	global
	juliaPath := FileSelect(,, "Locate julia.exe (C:\Users\UserName\AppData\Local\Julia-X.X.X\bin)")
	IniWrite juliaPath, configFile, "path", "JuliaPath"
}

startJulia() {
	global
	run(A_ScriptDir '\ConEmu\ConEmu64.exe -run "' juliaPath '"')
	WinWait(cmdProgram)
